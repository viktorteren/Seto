...
map<int, set<Region *> *> *
Merging_Minimal_Preregions_module::get_merged_preregions_map() {
    return merged_pre_regions_map;
}

map<int, set<Region *> *> *
Merging_Minimal_Preregions_module::get_total_preregions_map() {
                ...
                if (!Utilities::are_equals(reg1, reg2)) {
                    ...
                    for (auto record : *total_pre_regions_map) {
                        ...
                        for (auto reg : *total_pre_regions_map->at(event)) {
                            if (!Utilities::are_equals(reg, reg1) &&
                                !Utilities::are_equals(reg, reg2))
                                tmp_set->insert(reg);
                            else
                                event_contains_reg = true;
                        }
                        // inserisco l'unione se l'evento conteneva almeno una delle 2
                        // regioni da unire
