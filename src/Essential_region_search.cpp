...
set<Region *> *Essential_regions_search::search() {
   ...
   *      se c'è una sola regione che non ha tale stato allora la regione è
   * essenziale
   */
  ...
  for (auto record : *pre_regions) {
    ...
    if (record.second->size() == 1) {
      ...
      for (auto state : *temp_union) {
        ...
        // se ho avuto un solo stato candidato per essere essenziale allora è
        // davvero essenziale
        if (counter == 1) {
...
map<int, set<Region *> *> *
Essential_regions_search::get_essential_regions_map() {
