    ...
    if (argc >= 3) {
        ...
        for(int i=2; i < argc; i++) {
            ...
            /*if (args[i] == "L"){
                ...
                if(i < argc) {
                    if(is_number(args[i])) {
                        stringstream intValue(args[i]);
                        intValue >> repeat_counter;
                        if (repeat_counter < 1) {
                            cerr << "The number after REPEAT flag has to be greater than 0." << endl;
                            exit(1);
                        }
                    }
                    else{
                        cerr << "After REPEAT flag there should be a number greater than 0." << endl;
                        exit(1);
                    }
                }
                else{
                    cerr << "REPEAT flag needs a number of iterations." << endl;
                    exit(1);
                }
            }
            else{
            ...
            do {
                ...
                if (!excitation_closure) {
                    ...
                    if (!pre_regions->empty())
                        ls->split_ts_map(candidate_regions, aliases, rg->get_violations_event(),
                                         rg->get_violations_trans(),
                                         nullptr);
                    else
                        ls->split_ts_map(candidate_regions, aliases, rg->get_violations_event(),
                                         rg->get_violations_trans(),
