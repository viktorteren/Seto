...
k_FCPN_decomposition_with_levels::k_FCPN_decomposition_with_levels(int number_of_ev,
                                           set<Region *> *regions,
                                           const string& file,
     ...
     *          else
     *              calculate all unions of current regions and add the derived sets which satisfies ER(e)
     ...
     * while current FCPN set size < previous FCPN set size
    ...
    map<int, vector<set<Region *>*>*> *map_of_candidates;
    ...
    for(auto rec: *pre_regions_map){
        ...
        if(checked_ers->find(*(*ER_map)[ev]) == checked_ers->end()) {
            (*map_of_candidates)[ev] = new vector<set<Region *> *>();
            for (auto reg: *rec.second) {
                auto temp_set = new set<Region *>();
                temp_set->insert(reg);
                (*map_of_candidates)[ev]->push_back(temp_set);
            }
            for (auto temp_set: *(*map_of_candidates)[ev]) {
    ...
    cout << "all intersections found at lvl 0" << endl;
    ...
    cnf_ec_map = new map<int, set<set<Region *>*>*>();
    ...
    regions_in_conflict = new set<pair<Region *, Region *>>();
    for(auto rec: *pre_regions_map) {
        //auto ev = rec.first;
    ...
    state_regions_map = new map<int, set<Region*>*>();
    ...
    bool sufficient_sets = true;
    for(auto rec: *er_satisfiable_sets){
        if(rec.second->empty()){
            sufficient_sets = false;
            break;
        }
    }

    while(!sufficient_sets){
        level++;
        auto new_vector_of_candidates = new vector<set<Region *> *>();
        for (int i = 0; i < vector_of_candidates->size(); ++i) {
            for (int k = i + 1; k < vector_of_candidates->size(); k++) {
                auto reg_union = regions_set_union(vector_of_candidates->at(i), vector_of_candidates->at(k));
                new_vector_of_candidates->push_back(reg_union);
            }
        }
        ...
        for (auto rec: *pre_regions_map) {
            ...
            if (checked_ers->find(*(*ER_map)[ev]) == checked_ers->end()) {
                ...
                //cout << "EVENT " << ev << endl;
                for (auto temp_set: *new_vector_of_candidates) {
        ...
        for (auto elem: *new_vector_of_candidates) {
            vector_of_candidates->push_back(elem);
        }
        new_vector_of_candidates->clear();
        cout << "initial computation finished: level " << level << endl;
    }

    //first try
    compute(file,pprg);

    int previous_solution_size;
    int last_solution_size = number_of_FCPNs;

    cout << "search in deep start" << endl;
    //todo:here the do while cycle with different approach: check the previous lvl of regions guardare sufficient sets
    do{
        previous_solution_size = last_solution_size;
        auto new_vector_of_candidates = new vector<set<Region *> *>();
        for (int i = 0; i < vector_of_candidates->size(); ++i) {
            for (int k = i + 1; k < vector_of_candidates->size(); k++) {
                auto reg_union = regions_set_union(vector_of_candidates->at(i), vector_of_candidates->at(k));
        ...
        new_vector_of_candidates->clear();
        compute(file,pprg);
        last_solution_size = number_of_FCPNs;
    }while(previous_solution_size > last_solution_size);
    ...
    delete pre_regions_map;

    //delete set_of_initial_regions;
}

void k_FCPN_decomposition_with_levels::compute(const string& file, Pre_and_post_regions_generator *pprg){
    vector<int32_t> *clause;
    set<int32_t> *lit_set;
    bool solution_found = false;
    set<set<int32_t> *> *updatable_clauses = nullptr;
    while (!solution_found) {
        ...
        cout << "computed a try with " << number_of_FCPNs << " FCPNs" << endl;
        number_of_FCPNs++;
        /*for(auto cl: *updatable_clauses){
            delete cl;
        }
...
k_FCPN_decomposition_with_levels::~k_FCPN_decomposition_with_levels(){
    delete reg_map;
...
int k_FCPN_decomposition_with_levels::encoded_region(Region *reg, int current_FCPN_number) const{
...
pair<int, Region *> k_FCPN_decomposition_with_levels::decoded_region(int encoded_value) const{
    if(encoded_value < 0)
        encoded_value*=-1;
    int fcpn_counter = 0;
...
int k_FCPN_decomposition_with_levels::encoded_event(int event, int current_FCPN_number) const {
...
pair<int, int> k_FCPN_decomposition_with_levels::decoded_event(int encoded_value) const{
    if(encoded_value < 0)
        encoded_value*=-1;
